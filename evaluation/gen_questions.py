...
# The v6 answer contract and the gender rule, both imported from `app/` for the
# same reason as everything above: the stand must read the model's answer with
# the parser the endpoint uses and must state the same gender the endpoint
# states, or its bytes and its numbers describe a different endpoint (ClickUp
# 86cbejvt2). Both modules import nothing but each other's dependency-free
# helpers.
from person_gender import detect_gender  # noqa: E402
...
def used_subjects(
    ...
    """What this prayer has already asked about — the endpoint's own list.

    Mirrors `twinkler_ai.used_subjects`: one entry per question the person has
    been shown (the `assistant` turns of the request, then its
    `skipped_questions`), named by the subject the model committed to when we
    generated it and otherwise by an excerpt of the question itself.

    `subjects` is this run's stand-in for `question_format.SubjectMemory`:
    `question text -> subject`, filled from the rows this tool has already
    written. It is keyed by the raw text and looked up through the production
    normalisation, so a series step names the subjects of the steps before it —
    which is exactly what the endpoint does, because those questions were
    generated by the endpoint too. Questions that come from the INPUT file were
    never generated here (nor there), so they degrade to an excerpt on both
    sides.
    """
    memory = question_format.SubjectMemory()
    for question, subject in (subjects or {}).items():
        memory.remember(question, subject)
    questions = [
        text for role, text in turns(entry) if role == ROLE_ASSISTANT
    ] + list(skipped_questions or ())
    return [memory.subject_of(question) for question in questions]
...
def user_message(
    ...
    return question_prompts.user_message(
        ...
        language,
        person_gender(entry),
        used_subjects(entry, skipped_questions, subjects),
    )


def person_gender(entry: dict) -> str | None:
    """The grammatical gender the message states (prompt v6, 86cbejvt2).

    Mirrors `twinkler_ai.person_gender_texts` + `person_gender.detect_gender`:
    the person's `user` turns and the topic, never a question of ours. The
    decision function itself is imported, so only the SELECTION is mirrored
    here — and `tests/test_gen_questions.py` pins that against the endpoint on
    every probe, the way it pins `language_source`.
    """
    replies = [text for role, text in turns(entry) if role == "user"]
    return detect_gender(replies + [entry["topic"]])


def read_question(raw: str, variant: str) -> str:
    """What the person is shown, given one provider answer.

    On a v6 variant that is the `question` field of the JSON object
    (`app/question_format.parse_question`, the production parser); on every
    earlier wording the answer IS the question and is returned unchanged, so
    the artifacts of v1..v5 keep being produced by the code that produced them.
    """
    if not question_prompts.structured_answer(variant):
        return raw
    return parse_question(raw).question
...
def _replay_production_retry(
    ...
    Returns `(raw_texts, selection, extra_calls)`: `raw_texts[0]` is the first
    provider answer **verbatim** and `raw_texts[1]`, when present, the second
    generation. Every rule below is applied to the QUESTION inside them
    (`read_question`), exactly as the handler applies it to the parsed field
    rather than to the envelope; the raws are returned so the caller can record
    what the model actually sent.
    """
    first = read_question(first_raw, variant)
    if tier2_replaces(checked, first):
        # Production returns the fixed reply here and never retries.
        return [first_raw], choose_candidate([first], shown, checked), []
    ...
    retry_message = user_message(
        ...
        variant,
        # The rejected question is named by its own subject when the model gave
        # one, exactly as `twinkler_ai` hands it to the retry.
        {**(subjects or {}), first: parse_question(first_raw).subject or ""},
    )
    ...
    second_raw = texts[0]
    second = read_question(second_raw, variant)
...
def generate_one(
    ...
    sent = user_message(entry, skipped_questions, variant, subjects)
    wanted = max(1, getattr(args, "candidates", 1))

    raw_texts, usage, attempts, last_error, latency = _provider_call(
        client, args, url, api_key, model, sent, prompt, wanted
    )
    calls = [_call_row(latency, usage, wanted, attempts, last_error)]
    # On a v6 variant the provider answers with a JSON object and the person is
    # shown its `question` field alone (ClickUp 86cbejvt2). Everything below —
    # the despair tiers, the repeat filter, the candidate choice, the artifact's
    # `text` — therefore works on the parsed question, exactly as the handler
    # does, while `raw_texts` keeps what the model actually sent. On every
    # earlier wording the two are the same string.
    texts = [read_question(raw, variant) for raw in raw_texts]
    raw_text = ""
    if texts:
        text = texts[0]
    ...
    if texts and experiment(args):
        ...
        if getattr(args, "retry_on_repeat", False):
            raw_texts, selection, extra = _replay_production_retry(
                client, args, url, api_key, model, entry, variant, prompt,
                skipped_questions, raw_texts[0], shown, checked, subjects,
            )
            texts = [read_question(raw, variant) for raw in raw_texts]
        ...
        raw_text = raw_texts[index] if index is not None else ""
        if index is None:
    ...
    if question_prompts.structured_answer(variant):
        # ClickUp 86cbejvt2, and only on a v6 run: `text` above is already the
        # `question` field, so these three say where it came from — the answer
        # verbatim, the subject the model committed to, and which rung of
        # `question_format.parse_question` read it. A row whose `format_parse`
        # is "raw" is a model that ignored the contract, which is the number
        # this measurement exists to produce.
        parsed = parse_question(raw_text) if raw_text else None
        record["raw_text"] = raw_text
        record["subject"] = parsed.subject if parsed is not None else None
        record["format_parse"] = parsed.kind if parsed is not None else None
...
def dry_run(args: argparse.Namespace, parser: argparse.ArgumentParser) -> int:
    ...
    for entry in inputs:
        ...
        print(
            "    gender stated to the model: "
            + (person_gender(entry) or "unknown (no gendered forms asked for)")
        )
        print(
            "    despair rule reads: "
        ...
        with httpx.Client(timeout=httpx.Timeout(args.timeout)) as client:
            for entry in inputs:
                for sample in range(1, args.samples + 1):
                    ...
                    replaced: list[str] = []
                    # This run's stand-in for the endpoint's subject memory:
                    # every question this series has produced, mapped to the
                    # subject the model named for it (prompt v6, 86cbejvt2).
                    ...
                    for step in range(1, entry.get("steps", 1) + 1):
                        ...
                        if record["text"]:
                            ...
                            if record.get("subject"):
                                subjects[record["text"]] = record["subject"]
