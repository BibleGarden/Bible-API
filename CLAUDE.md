...
- **`language_detection.py`** — the shared offline language detector (`py3langid==0.4.0`, bundled 139-language model, normalized probability `>= 0.9`): returns an ISO code or `None`, loads one immutable model eagerly at startup, and performs no network inference (ADR 0018)
- **`safety.py`** — the despair / self-harm rule of `POST /api/ai/question` in code rather than in the prompt (reviewed ru/uk/en dictionary + regex; language comes from the shared offline detector): tier 1 answers the versioned fixed reply (`SAFETY_REPLIES`, `SAFETY_REPLY_VERSION`) without calling the provider, tier 2 replaces a model reply that came back as a question for a weaker despair signal. Since 86cbegmzz both tiers read the person's last reply (the topic at `stage: first`). Tier 2 keeps a known `language_source`; when detection abstains it uses the already-matched pattern language, the same evidence rule as tier 1 (Maria, 2026-09-13; ADR 0018)
...
- **The contract does not change with the provider**: verbatim, in the
  ...
  spoken language detected from the audio; the optional locale remains accepted
  for client compatibility and is ignored. Same 200 shape, same
...
rest of the prayer. Tier 2's own fixed reply still needs a language. Known
context is resolved the same way the prompt's is: the last `user` turn,
...
rather than presence. The offline detector abstains below normalized
probability `0.9`, so a line like «Помоги» lets stronger evidence elsewhere in
the same request decide. If the whole chain remains unknown, tier 2 uses the
reviewed language of its matched safety pattern, as tier 1 does (ADR 0018).
The blocks the server assembles are Russian whatever the prayer's
